...
- **Three flavors of Copilot context**, side by side:
  ...
  |---|---|---|
  | **Instructions** (`AGENTS.md`, [ex 03](./03-custom-instructions.md)) | Always-on rules ("use reST docstrings") | Every prompt, automatically |
...
- **Skills compose with agents and instructions.** `AGENTS.md` still applies (so docstrings still follow exercise 03's format), and `/review` still runs on top of the diff.
